...
func main() {
	...
	if srcInfo.IsDir() && !*copyDirs {
		fmt.Println("Error: Source is a directory but --d flag is not set")
		return
	}
...
func copyDir(src string, dest string, overwrite bool, threshold int) error {
	...
	var fileCount int
	err = filepath.Walk(src, func(path string, info os.DirEntry, err error) error {
		if err != nil {
			return err
		}
...
func copyFile(src string, dest string, overwrite bool) error {
	if _, err := os.Stat(dest); err == nil && !overwrite {
		return fmt.Errorf("destination file '%s' already exists. Use --o to replace it", dest)
	}
